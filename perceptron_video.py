...
class PerceptronReel(Scene):
    ...
    def animation_train(self, plano, puntos, etiquetas, puntos_l, iteraciones=5):
        # Entrenamiento
        perceptron = Perceptron(2, puntos, etiquetas, 0.1)

        f_x, f_y = perceptron.frontera_decision()
        linea = Line((f_x[0], f_x[1], 0), (f_y[0], f_y[1], 0), color=RED, stroke_width=5)       
        lineas = VGroup(linea)

        pesos = [float(perceptron.weights[0]), float(perceptron.weights[1]), 0]
        vector_pesos = Vector(pesos, color=RED, stroke_width=5)
        vectores = VGroup(vector_pesos)
        self.play(Create(vector_pesos))
        self.wait()
        ...
        for i in range(iteraciones):
            ...
            if perceptron.predict(x) != y:
                ...
                pesos = [float(perceptron.weights[0]), float(perceptron.weights[1]), 0]
                vectores.add(Vector(pesos, color=random_bright_color(), stroke_width=5))
                ...
                self.play(
                    ...
                    ReplacementTransform(vectores[-2], vectores[-1]),
